...
function RobinhoodWebApi(opts, callback) {
  ...
  var _options = opts || {},
    ...
    _endpoints = {
      ...
      watchlists: 'midlands/lists/items/',
      positions: 'positions/',
  ...
  function get_options_details(options) {
    ...
    return Promise.all(grouped_options.map(group => {
  ...
  api.cancel_order = function (order, callback) {
    ...
    if (cancel_url) {
      ...
      return _request.post(
        // use the cancel url provided by the order object
        { uri: cancel_url },
        callback,
      );
    } else {
      return callback(
        {
          message:
            order.state === 'cancelled' ? 'Order already cancelled.' : 'Order cannot be cancelled.',
          order: order
        },
        null,
        null
  ...
  var _place_order = (options, callback) => {
    return _request.post(
      {
  ...
  api.url_post = function (url, body, callback) {
    return _request.post(
      {
        uri: url,
        body,
